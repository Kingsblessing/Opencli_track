...
def parse_relative_time(text: str, base: Optional[datetime.datetime] = None) -> Optional[str]:
    """'11小时前' / '3天前' -> ISO8601;绝对日期原样规范化;无法解析返回 None。"""
    if not text:
        return None
    ...
    if m:
        ...
        deltas = {"秒": datetime.timedelta(seconds=n), "分钟": datetime.timedelta(minutes=n),
                  "小时": datetime.timedelta(hours=n), "天": datetime.timedelta(days=n),
                  "周": datetime.timedelta(weeks=n), "个月": datetime.timedelta(days=30 * n),
                  "月": datetime.timedelta(days=30 * n), "年": datetime.timedelta(days=365 * n)}
        return (now - deltas[unit]).isoformat(timespec="seconds")
    for fmt in ("%Y-%m-%d %H:%M", "%Y-%m-%d", "%m-%d", "%Y-%m-%d %H:%M:%S"):
        try:
